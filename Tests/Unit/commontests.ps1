    ...
    Context 'All discovered ext files' {
        ...
        It 'Should not contain any files with tab characters' {
            ...
            foreach ($textFile in $textFiles)
            {
                ...
                $fileContent = Get-Content -Path $fileName -Encoding UTF8 -Raw
        ...
        It 'Should not contain empty files' {
            ...
            foreach ($textFile in $textFiles)
            {
                $fileContent = Get-Content -Path $textFile.FullName -Encoding UTF8 -Raw
    ...
    Context "AADSC Nodes" {
        ...
        foreach ($Node in $Nodes) {
            It "Node $($Node.Name) is compliant with $($Node.NodeConfigurationName)" {
                $Node.conversionstatus | Should Be 'Compliant'
            }
        }
    }
}    
